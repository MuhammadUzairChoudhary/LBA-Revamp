...
const InputField = ({ name, type, placeholder, error, register, maxLength, prefix }) => (
  <div>
    <div className="relative">
      {prefix && (
        <div className="absolute left-3 md:left-4 top-1/2 transform -translate-y-1/2 text-gray-600 font-light text-sm md:text-base lg:text-lg pointer-events-none">
          {prefix}
        </div>
      )}
      <input
        ...
        maxLength={maxLength}
        className={`
  w-full px-4 py-2 md:px-5 md:py-3 lg:py-3.5 rounded-xl transition-all duration-200 text-gray-900
  ...
  `}
    ...
    {error && <p className="mt-2 text-sm text-red-600 font-light">{error.message}</p>}
  </div>
)
    ...
    <div className="max-w-3xl mx-auto px-2 sm:px-4 py-6 sm:py-8">
      <div className="bg-white rounded-2xl p-5 sm:p-8 border border-gray-200 shadow-2xl shadow-gray-400/20">
        ...
        <form>
          ...
          <div className="flex justify-between gap-6 pt-6 sm:pt-8 border-t border-gray-200">
            ...
            <button
              ...
              className={`
    px-8 sm:px-10 py-2.5 sm:py-3 text-sm sm:text-base font-light rounded-lg 
